...
import {
  Activity, ArrowUpRight, BookOpen, Check, ChevronDown, Clock3, Copy, ExternalLink,
  FileSearch, Globe2, History, LayoutDashboard, Link2, Loader2, Menu, MoreHorizontal,
  PanelLeftClose, PanelLeftOpen, Play, RefreshCw, Search, Sparkles, TerminalSquare,
  Trash2, X, Zap,
} from 'lucide-react'

type View = 'overview' | 'scrape' | 'history'
    ...
    <div className="flex h-screen overflow-hidden bg-[#f8fafc] text-gray-900">
      <aside className={`${sidebarOpen ? 'w-[248px]' : 'w-[76px]'} hidden shrink-0 border-r border-gray-200 bg-white transition-all duration-300 md:flex md:flex-col`}>
        ...
        <nav className="flex-1 space-y-1 px-3 py-6">
          {([['overview', LayoutDashboard, 'Overview'], ['scrape', Sparkles, 'New scrape'], ['history', History, 'History']] as const).map(([id, Icon, label]) => (
